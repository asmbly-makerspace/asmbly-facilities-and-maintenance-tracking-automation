...
class TestSlackReorderWithDynamoDB(unittest.TestCase):
    ...
    def test_initial_open_invokes_async_lambda(self, mock_get_secret, mock_session_cls, mock_boto_client):
        ...
        mock_boto_client.return_value = mock_lambda_instance
        ...
        invoke_args = mock_lambda_instance.invoke.call_args.kwargs
    ...
    @patch("boto3.client")
    def test_view_submission_acknowledges_and_invokes_async(self, mock_boto_client):
        # GIVEN
        mock_lambda_instance = MagicMock()
        mock_boto_client.return_value = mock_lambda_instance
        payload = {"type": "view_submission", "user": {"id": "fake_user_id"}, "view": {"state": {"values": {}}}}
        ...
        self.assertIn("Processing", response_body['view']['title']['text'])
        
        mock_boto_client.assert_called_with('lambda')
        mock_lambda_instance.invoke.assert_called_once()
        invoke_args = mock_lambda_instance.invoke.call_args.kwargs
        self.assertEqual(json.loads(invoke_args['Payload']), {"action": "process_submission", "payload": payload})
    ...
    @patch(f"{LAMBDA_FUNCTION_PATH}.requests.Session")
    def test_handle_process_submission_creates_task_and_updates_view(self, mock_session_cls, mock_create_task, mock_get_task, mock_get_slack_user_info, mock_get_secret, mock_get_json_param):
        # GIVEN
        mock_session_cls.return_value = self.mock_http_session
        ...
        # WHEN
        lambda_function.handle_process_submission(payload)
        ...
        self.assertIn("views.update", call_args[0][0])
        sent_view = call_args[1]['json']['view']
        self.assertEqual(sent_view['title']['text'], 'Success!')
